...
router.post("/add", (req, res) => {
  ...
  jwt.verify(token.split(" ")[1], process.env.JWT_SECRET, (err, decode) => {
    ...
    conn.query(
      `INSERT INTO categories(title,status,cover,description,refer_to,created_at,updated_at)
          VALUES('${req.body.title}','${req.body.status}','${req.body.cover}','${req.body.description}','${refer_to}','${today}','${today}')`,
      (err, result) => {
        if (err) {
          res.json({
            success: "false",
